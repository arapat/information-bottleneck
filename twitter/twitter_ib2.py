...
def gen_data(docs, all_tags, all_pairs):
  ...
  log.info("(batch) number of tags: %d" % (ntags))
...
def run_streaming(alpha, beta, converge_dist, batchSize, numOfBatches, \
    initBatches, usertagsIndex):
  docs = get_data(initBatches - 1, batchSize, usertagsIndex, fromStart = True)
  idtags, ratio_t, ratio_p, all_tags, all_pairs, \
      c_h, bp_h, p_uh, p_uh_co_occur = gen_data(docs, 0, 0)
  p_t, p_yt_co_occur, centroid_id = init_centroids(c_h, bp_h.value, p_uh, p_uh_co_occur, beta)
  log.info("Initial centroid: " + idtags[centroid_id])

  all_tags, all_pairs = 0, 0
  old_data = sc.parallelize([])
  batchId = 0
  while batchId < numOfBatches and (batchId + 1) * batchSize <= numOfDocs:
    # Create batch
    timer = time()
    ...
    idtags, ratio_t, ratio_p, all_tags, all_pairs, c_h, bp_h, p_uh, p_uh_co_occur = \
        gen_data(docs, all_tags, all_pairs)
...
batchSize = 25000
sampleBatches = 10
p_t, p_yt_co_occur = run_streaming(alpha, beta, converge_dist, batchSize, numOfBatches, sampleBatches, usertagsIndex)
